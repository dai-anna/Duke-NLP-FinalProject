...
\section{Discussion}
Our results indicate that model performance is determined by a combination of the type and quality of data the model is trained on and the expressiveness of the model. Our models that trained explicitly on synthetic data performed well on predicting synthetic test data but poorly on the real data, and vice versa. In addition, our more expressive neural networks performed better to varying extents than the LDA model in predicting topics in the real data.
