...
#%%
TRAIN_ON_SYNTHETIC = False  # Set to True to train on synthetic data
CONTINUE_ON_REAL = False  # Set to True to continue training on real data. Needs TRAIN_ON_SYNTHETIC to be True.
JUST_ON_REAL = True  # Set to True to ONLY train on real data.

assert TRAIN_ON_SYNTHETIC or CONTINUE_ON_REAL or JUST_ON_REAL
assert not (TRAIN_ON_SYNTHETIC and JUST_ON_REAL)
assert not (CONTINUE_ON_REAL and JUST_ON_REAL)
...
if CONTINUE_ON_REAL:
    ...
    xval = nn.utils.rnn.pad_sequence(sequences=xval, batch_first=True, padding_value=0.0)
    xtest = nn.utils.rnn.pad_sequence(
        sequences=xtest, batch_first=True, padding_value=0.0
    )
...
if JUST_ON_REAL:
    ...
    model.save_weights("../artefacts/model_just_realdata.hdf5")
